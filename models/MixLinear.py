"""MixLinear (ICLR 2026) -- Extreme Low-Resource Multivariate Time Series Forecasting
with 0.1K Parameters. Ma, Luo & Sha. https://arxiv.org/abs/2410.02081

Source: https://github.com/aitianma/MixLinear (models/MixLinear.py)

Adapted for this repository:
  * forward() takes the TSLib signature (x_enc, x_mark_enc, x_dec, x_mark_dec, mask)
    instead of forward(x); the extra arguments are unused, as in the original, which
    conditions only on the lookback window.
  * Debug print() calls removed from the forward pass.
  * Unused `from layers.Embed import PositionalEmbedding` dropped.
The model arithmetic is otherwise unchanged.

Config fields: seq_len, pred_len, enc_in, period_len, lpf, alpha.
"""
...
class Model(nn.Module):
    ...
    def forecast(self, x):
        ...
        x_o = x_o.reshape(
            batch_size, self.enc_in, self.period_len,
        ...
        x_fft = self.FLinear2(x_fft).reshape(
            batch_size, self.enc_in, self.period_len, -1
        )
        x_rfft = torch.fft.ifft(x_fft, dim=3).float()
        x_f = (
            x_rfft.permute(0, 1, 3, 2)
            .reshape(batch_size, self.enc_in, -1)
            .permute(0, 2, 1)
        )
        ...
        # Mix
        x = (
            x_t[:, : self.pred_len, :] * self.alpha
            + seq_mean
            + x_f[:, : self.pred_len, :] * (1 - self.alpha)
        )
